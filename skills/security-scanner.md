...
1. Read the diff file and scan every patch for the following.
   Flag ALL matches 鈥?do not skip any.

2. CRITICAL 鈥?Hardcoded secrets (flag any line matching these patterns):
...
3. CRITICAL 鈥?Injection vulnerabilities:
...
5. SUGGESTION 鈥?Security hygiene:
...
## Error Handling
- Binary or minified files 鈫?skip
- If no security issues found 鈫?explicitly state "No security issues detected"
